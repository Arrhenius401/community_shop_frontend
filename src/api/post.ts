...
/**
 * 获取私有帖子列表（对应后端 GET /api/v1/posts/query/list/private，需登录）
 * @param params 帖子查询参数（与后端 PostQueryDTO 对齐，含分页）
 * @returns 分页帖子列表
 */
export const queryPrivatePostList = (params: PostQueryParams) => {
  return request.get<PostListPageResult>('/api/v1/posts/query/list/private', { params });
}

/**
 * 统计符合条件的私有帖子数量（对应后端 GET /api/v1/posts/query/count/private，需登录）
 * @param params 帖子查询参数（与后端 PostQueryDTO 对齐）
 * @returns 符合条件的帖子数量
 */
export const queryPrivatePostCount = (params: PostQueryParams) => {
